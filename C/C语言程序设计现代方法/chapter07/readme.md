...
- 允许程序获取存储指定类型的值所需要的内存空间的字节数
- 格式: sizeof (类型名)
- 类型是 size_t,一种由实现定义的类型
