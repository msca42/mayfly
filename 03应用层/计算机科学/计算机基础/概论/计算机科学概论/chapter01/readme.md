...
- 位的存储和操作
  ...
  - 布尔运算：位的操作
    ...
    - NOT
      ...
      - 输入与输出值相反
- 门
  - 布尔运算的硬件实现，可以通过齿轮、继电器和光学设备实现
  - 给定一种布尔运算的输入值，可以得到该布尔运算的输出值
  - 现代计算机中的门通常由晶体管这种小型电路实现
  - 分类
    - 与门、或门、异或门、非门
      ![门](./门.png)
...
- 触发器
  - 由门电路组成
  - 计算机存储器的基本单元，可以产生 0 或 1 输出值的电路
  - 它的值会保持不变，直到另一个电路的脉冲信号使其变换为其他值,也就是说输出可以在外界刺激下记住 0 或 1 作为输出值
