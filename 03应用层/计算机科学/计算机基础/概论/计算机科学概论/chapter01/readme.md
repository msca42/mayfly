...
- 十六进制记数法
  ...
  - 为了简化位模式的表示
  - 位模式也称为位串，长位串称为流，通常使用十六进制简写位串，一个十六进制表示位模式的四位

- 问题与练习
  - Q1
    - 输出 1 意味与门输入都是 1，第三输入为 1，异或门输出为 1，后者意味第一输入和第二输入不同，一个 1 一个 0
  - Q2
    - 上输入为 0，下输入为 1，触发器输出为 0
    - 事件序列
      - 下输入为 1，NOT 门输出为 0，与门输入为 0，与门输出为 0，输出为 0
