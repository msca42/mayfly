...
- 用户样式
  - 用户定义自己的样式
- 样式如何层叠
  - 样式的显示次序：元素内嵌、文档内嵌、外部样式、用户样式、浏览器样式
- 用重要样式调整层叠次序
...
- 相对长度
  - 依托其他类型的单位
  - em、ex、rem、px、%
  - em:相对于父元素的字号 font-size 的倍数
  - rem:相对于根元素的字号 font-size 的倍数
  - ex：相当于当前字体 x 的高度，也就是基线到中线之间的距离，1ex 相当于 0.5em
  - 百分比: 不同属性的百分比对象不同
    - font-size 属性挂钩的是继承到的 font-size 值
    - width 属性挂钩的是元素的包含块的宽度
- 像素单位
  - px:被当作绝对单位，默认16px
- 其他单位
  - gd、vw、vh、vm、ch
- 算式做值
  - 在 CSS3 中，引入了一个特性，算式可以作为属性值
